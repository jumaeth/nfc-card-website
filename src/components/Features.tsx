...
import { features, ui } from "@/lib/site";
    ...
    <section id="features" className="section-pad relative overflow-hidden bg-ink py-24 text-paper sm:py-32">
      ...
      <div className="relative">
        <Reveal>
          <SectionHeading
            dark
            eyebrow={t(ui.features.eyebrow)}
            title={t(ui.features.title)}
            intro={t(ui.features.intro)}
          />
        </Reveal>
          ...
          {features.map((f, i) => (
            <Reveal key={f.key} delay={(i % 3) * 0.08}>
              <div className="group h-full bg-ink p-8 transition-colors hover:bg-ink-2">
                ...
                <h3 className="display mt-6 text-xl text-paper">{t(f.title)}</h3>
                <p className="mt-2 leading-relaxed text-paper/55">{t(f.body)}</p>
              </div>
            </Reveal>
          ))}
        </div>
